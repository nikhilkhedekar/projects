...
function getPaymentItemsHTML(cartItems){
    let data = ""
    for(let item of cartItems){
        data += `
    <div class="table-row">
        <div class=" table-cell w-6/12 text-left font-bold py-1 px-4">${item.name}</div>
        <div class=" table-cell w-[10%] text-center">${item.amount}</div>
        <div class=" table-cell w-2/12 text-center">₹${item.price}</div>
    </div>
    `
    }
    return data
        ...
        <div class="flex items-start justify-center">
            <div class="flex-1">
                ...
                <div class="w-60 pl-4 pb-6">
                    <h3 class="font-bold">${order.user}</h3>             
                </div>
                
                <div class="pl-4 pb-20">
                    <p class="text-gray-500">Receipt To:</p>
                    <h3 class="font-bold">${order.user}</h3>
                </div>
            ...
            <div class="flex items-end flex-col">
                <div class="pb-16">
                    <h1 class=" font-normal text-4xl pb-1">Payment Receipt</h1>
                    <p class="text-right text-gray-500 text-xl"># ${order._id}</p>
                </div>
                <div class="flex">
                    <div class="flex flex-col items-end">
                        <p class="text-gray-500 py-1">Payment Reference id:</p>
                        <p class="text-gray-500 py-1">Date:</p>                        
                        <p class="text-gray-500 py-1">Total Qauntity:</p>
                        <p class="text-gray-500 py-1">Tax:</p>
                        <p class="text-gray-500 py-1">Shipping Fee:</p>
                        <p class="text-gray-500 py-1">Subtotal:</p>
                        <p class="font-bold text-xl py-1 pb-2 ">Paid Amount:</p>
                    </div>
                    <div class="flex flex-col items-end w-[12rem] text-right">
                        <p class="py-1 pl-10">${order.stripe_payment_intent_id}</p>
                        <p class="py-1">${order.createdAt}</p>
                        <p class="py-1 pl-10">${order.cartQty}</p>
                        <p class="py-1 pl-10">${order.tax}</p>
                        <p class="py-1 pl-10">${order.shippingFee}</p>
                        <p class="py-1 pl-10">${order.subtotal}</p>
                        <div class="pb-2 py-1">
        ...
        <div class="table w-full">
            <div class=" table-header-group bg-gray-700 text-white ">
                <div class=" table-row ">
                    ...
                    <div class=" table-cell w-2/12 text-center border-x-[1px]">Rate</div>s                    
                </div>
            </div>
            <div class="table-row-group">
                ${getPaymentItemsHTML(order.cartItems)}
            </div>
        </div>
        
        <!--Total Amount-->
        <div class=" pt-20 pr-10 text-right">
            <p class="text-gray-400">Total: <span class="pl-24 text-black">₹${order.total}</span></p>
        </div>
    </div>
</body>
</html>
...
async function getPaymentReceipt(order) {
    return new Promise(async (resolve, reject) => {
        try {
            const html = getPaymentHTML(order);
